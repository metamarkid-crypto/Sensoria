...
export interface RoyaltyRecord {
  monthKey: string;
  revenue: number;
  royalty: number; // 10% of realized revenue
}
 ...
 * Royalty is computed from PAID transactions in the calendar month —
 * pending money is never royalty-eligible.
 */
export async function getExecutiveData(): Promise<{
  ...
  const [subs, tx, devices] = await Promise.all([
  ...
  // ── KPIs ────────────────────────────────────────────────────────────────
  const paidTx = txRows.filter((t) => t.status === "paid");
  ...
  // ── Monthly revenue + royalty history (realized money only) ────────────
  ...
  const revenueSeries: RevenuePoint[] = [...byMonth.entries()]
    ...
    .map(([monthKey, revenue]) => ({
      monthKey,
      revenue,
      royalty: revenue * MONTHLY_ROYALTY_RATE,
    }));
